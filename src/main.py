from __future__ import print_function, division
import networkx as nx
import numpy as np
import pandas as pd
from sklearn.ensemble import RandomForestClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.svm import SVC
from xgboost import XGBClassifier
from sklearn.naive_bayes import GaussianNB
from sklearn.tree import DecisionTreeClassifier
from sklearn.preprocessing import StandardScaler
import os
import glob
from pathlib import Path



def extract_causal_ancestors_only(gt_graph_edges, target_node="safety"):
    if not gt_graph_edges:
        return set()

    G = nx.DiGraph()
    for edge in gt_graph_edges:
        if '->' in edge:
            u, v = edge.split('->', 1)
            u, v = u.strip(), v.strip()
            G.add_edge(u, v)

    if target_node not in G:
        return set()

    ancestors = set()
    for node in G.nodes:
        if node == target_node:
            continue
        if nx.has_path(G, node, target_node):  # node → ... → safety
            ancestors.add(node)
    return ancestors


def get_models(random_state):
    return {
        "LogisticRegression": LogisticRegression(max_iter=1000, random_state=random_state),
        "RandomForest": RandomForestClassifier(n_estimators=100, random_state=random_state),
        "SVM": SVC(probability=True, random_state=random_state),
        "XGBoost": XGBClassifier(eval_metric='logloss', random_state=random_state),
        "NaiveBayes": GaussianNB(),
        "DecisionTree": DecisionTreeClassifier(random_state=random_state)
    }


def load_and_preprocess(file_path):
    df = pd.read_csv(file_path, on_bad_lines='warn')
    df = df.drop(columns=['start_time', 'end_time'], errors='ignore')

    if 'safety' not in df.columns:
        raise ValueError(f" {file_path}  'safety'")

    df['safety'] = df['safety'].map({'safe': 0, 'unsafe': 1})
    if df['safety'].isnull().any():
        raise ValueError(f"{file_path} 'safety'")

    feature_columns = [col for col in df.columns if col != 'safety']
    df[feature_columns] = df[feature_columns].apply(pd.to_numeric, errors='coerce')

    if df[feature_columns].isnull().any().any():
        problematic_cols = df[feature_columns].columns[df[feature_columns].isnull().any()]
        raise ValueError(f"{file_path}: {problematic_cols.tolist()}")

    return df


def calculate_apfd_c(safety_series, cost_series):
    safety_series = np.array(safety_series)
    cost_series = np.array(cost_series)
    cumulative_cost = np.cumsum(cost_series)
    cumulative_faults = np.cumsum(safety_series == 1)
    area = np.trapz(cumulative_faults, cumulative_cost)
    max_cost = np.sum(cost_series)
    max_faults = np.sum(safety_series == 1)
    max_area = max_cost * max_faults if max_faults > 0 else 0
    return area / max_area if max_area > 0 else 0


def calculate_recall_effort_at_20(sorted_safety, sorted_duration):
    safety = np.array(sorted_safety)
    duration = np.array(sorted_duration)

    total_time = duration.sum()
    total_faults = (safety == 1).sum()

    if total_faults == 0 or total_time == 0:
        return 0.0, 0.0

    # ---------- Recall@20 ----------
    time_20 = 0.2 * total_time
    cumulative_time = np.cumsum(duration)
    idx_20 = np.searchsorted(cumulative_time, time_20)
    found_faults_20 = (safety[:idx_20 + 1] == 1).sum()
    recall20 = found_faults_20 / total_faults

    # ---------- Effort@20 ----------
    target_faults_20 = 0.2 * total_faults
    cumulative_faults = np.cumsum(safety == 1)
    idx_faults_20 = np.searchsorted(cumulative_faults, target_faults_20)
    time_to_find_20faults = cumulative_time[min(idx_faults_20, len(cumulative_time) - 1)]
    effort20 = time_to_find_20faults / total_time

    return recall20, effort20


def evaluate_transfer_causal(src_data, tgt_data, model, causal_feature_set=None):
    try:
        feature_cols = [col for col in src_data.columns if col != 'safety']
        if causal_feature_set is not None:
            feature_cols = [f for f in feature_cols if f in causal_feature_set]
            if not feature_cols:
                return 0.0, 0.0, 0.0

        X_src = src_data[feature_cols]
        y_src = src_data['safety']
        X_tgt = tgt_data[feature_cols]
        y_tgt = tgt_data['safety']

        common_features = X_src.columns.intersection(X_tgt.columns)
        if len(common_features) == 0:
            return 0.0, 0.0, 0.0

        X_src = X_src[common_features]
        X_tgt = X_tgt[common_features]

        scaler = StandardScaler()
        X_src_scaled = scaler.fit_transform(X_src)
        X_tgt_scaled = scaler.transform(X_tgt)

        model.fit(X_src_scaled, y_src)

        if hasattr(model, "predict_proba"):
            y_prob = model.predict_proba(X_tgt_scaled)[:, 1]
        else:
            y_prob = model.decision_function(X_tgt_scaled)
            y_prob = (y_prob - y_prob.min()) / (y_prob.max() - y_prob.min() + 1e-8)

        epsilon = 1e-8
        if "duration_seconds" in tgt_data.columns:
            duration = tgt_data["duration_seconds"].replace(0, epsilon)
            score = y_prob / duration
        else:
            score = y_prob

        sorted_indices = np.argsort(-score)
        sorted_safety = y_tgt.values[sorted_indices]
        if "duration_seconds" in tgt_data.columns:
            sorted_duration = tgt_data["duration_seconds"].values[sorted_indices]
        else:
            sorted_duration = np.ones_like(sorted_safety)

        apfd_c = calculate_apfd_c(sorted_safety, sorted_duration)
        recall20, effort20 = calculate_recall_effort_at_20(sorted_safety, sorted_duration)

        return apfd_c, recall20, effort20

    except Exception as e:
        print(f"❌ evaluate_transfer_causal: {e}")
        return 0.0, 0.0, 0.0


CAUSAL_ROOT_DIR = "../multi_causal_results"
GROUND_TRUTH_DIR = "../ground_truth_graph"

LEARNERS = [
    "LogisticRegression", "RandomForest", "SVM", "XGBoost", "NaiveBayes", "DecisionTree"
]


def normalize_edge(edge_str):
    if not isinstance(edge_str, str):
        return ""
    if ':' in edge_str:
        edge_str = edge_str.split(':', 1)[0]
    edge_str = edge_str.replace(' ', '').strip()

    if '->' not in edge_str:
        return edge_str

    parent, child = edge_str.split('->', 1)

    def normalize_node(node):
        node = node.strip()
        if node == 'prob':
            return 'safety'
        elif node == 'test_duration':
            return 'duration_seconds'
        else:
            return node

    return f"{normalize_node(parent)}->{normalize_node(child)}"


def load_graph_from_txt(file_path):
    if not os.path.exists(file_path):
        return set()
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = [line.strip() for line in f if line.strip()]
        edge_lines = lines[1:] if lines and lines[0].isdigit() else lines
        edges = set()
        for line in edge_lines:
            if '->' in line:
                edge_str = normalize_edge(line)
                edges.add(edge_str)
        return edges
    except Exception:
        return set()


def jaccard_similarity(set1, set2):
    if len(set1) == 0 and len(set2) == 0:
        return 1.0
    union = len(set1 | set2)
    return len(set1 & set2) / union if union > 0 else 0.0


def get_causal_models():
    causal_dirs = glob.glob(os.path.join(CAUSAL_ROOT_DIR, "*_prediction"))
    models = []
    for d in causal_dirs:
        name = os.path.basename(d)
        if name.endswith('_prediction'):
            model_name = name[:-11]
            models.append(model_name)
    return sorted(set(models))


def parse_csv_name(filename):
    if not filename.endswith('.csv'):
        return None, None
    name_without_ext = filename[:-4]
    if name_without_ext.endswith('_Complete'):
        variant = 'Complete'
        base = name_without_ext[:-9]
    elif name_without_ext.endswith('_selected'):
        variant = 'selected'
        base = name_without_ext[:-9]
    else:
        return None, None
    if not base or base.endswith('_'):
        return None, None
    return base, variant


def discover_all_targets():
    gt_files = (
            glob.glob(os.path.join(GROUND_TRUTH_DIR, "*_Complete_graph.txt")) +
            glob.glob(os.path.join(GROUND_TRUTH_DIR, "*_selected_graph.txt"))
    )
    targets = []
    for f in gt_files:
        stem = Path(f).stem
        if not stem.endswith('_graph'):
            continue
        name_without_graph = stem[:-6]
        parts = name_without_graph.rsplit('_', 1)
        if len(parts) != 2:
            continue
        base, variant = parts
        if variant not in ['Complete', 'selected']:
            continue
        full_name = f"{base}_{variant}"
        targets.append(full_name)
    return sorted(set(targets))


def get_causal_model_ground_truth(causal_model_name, target_full):
    gt_dir = os.path.join(GROUND_TRUTH_DIR, f"{causal_model_name}_ground_truth_graph")
    candidates = [f"{target_full}_graph.txt", f"{target_full}.txt"]
    for filename in candidates:
        gt_file = os.path.join(gt_dir, filename)
        if os.path.exists(gt_file):
            return load_graph_from_txt(gt_file)
    return set()


def get_predicted_graph(target_full, source_full, learner, causal_models, base_variant_to_base):
    target_base = base_variant_to_base[target_full]
    source_base = base_variant_to_base[source_full]
    source_variant = 'Complete' if source_full.endswith('_Complete') else 'selected'

    for model in causal_models:
        pred_dir = os.path.join(CAUSAL_ROOT_DIR, f"{model}_prediction", f"graph_all_{target_base}")
        if not os.path.exists(pred_dir):
            continue
        for suffix in ["", "_graph"]:
            pred_file = os.path.join(pred_dir, f"{source_base}_{source_variant}_{learner}{suffix}.txt")
            if os.path.exists(pred_file):
                return load_graph_from_txt(pred_file)
    return set()


def calculate_overall_match_ratio(src, tgt, learner, causal_models, base_variant_to_base):
    total_union_edges = 0
    total_correct_predictions = 0

    for model in causal_models:
        true_graph = get_causal_model_ground_truth(model, src)
        pred_graph = get_predicted_graph(tgt, src, learner, [model], base_variant_to_base)

        same_edges = len(true_graph & pred_graph)
        union_edges = len(true_graph | pred_graph)

        total_union_edges += union_edges
        total_correct_predictions += same_edges

    overall_match_ratio = total_correct_predictions / total_union_edges if total_union_edges > 0 else 0.0
    return overall_match_ratio


def min_max_norm(x):
    x_min, x_max = x.min(), x.max()
    if x_max - x_min < 1e-8:
        return np.ones_like(x)
    return (x - x_min) / (x_max - x_min + 1e-8)


def load_projects(data_dir):
    csv_files = glob.glob(os.path.join(data_dir, "*.csv"))

    projects = {}
    base_variant_to_base = {}

    for file_path in csv_files:
        filename = os.path.basename(file_path)
        base, variant = parse_csv_name(filename)
        if base is None:
            continue
        full_name = f"{base}_{variant}"
        try:
            projects[full_name] = load_and_preprocess(file_path)
        except Exception as e:
            continue
        base_variant_to_base[full_name] = base

    if not projects:
        raise ValueError("No CSV projects!")

    valid_targets = sorted(projects.keys())
    print(f"\n✅ ({len(valid_targets)}): {valid_targets}")

    return projects, base_variant_to_base, valid_targets


def compute_causal_features(projects, valid_targets, causal_models):
    source_causal_features = {}

    for src in valid_targets:
        all_ancestors = set()
        for model in causal_models:
            gt_graph = get_causal_model_ground_truth(model, src)
            ancestors = extract_causal_ancestors_only(gt_graph, "safety")
            all_ancestors.update(ancestors)
        source_causal_features[src] = all_ancestors
        print(f"  {src}:  {len(all_ancestors)} feature remained")

    return source_causal_features


def save_causal_features(source_causal_features, output_dir):
    feature_log_df = pd.DataFrame([
        {'Source': s, 'Causal_Ancestors': ';'.join(sorted(f)), 'Num_Causal_Ancestors': len(f)}
        for s, f in source_causal_features.items()
    ])
    feature_log_df.to_csv(os.path.join(output_dir, "source_causal_ancestors.csv"), index=False)


def compute_avg_jaccard(projects, valid_targets, base_variant_to_base, causal_models):
    avg_jaccard_cache = {}  
    match_ratio_detail_records = []

    for tgt in valid_targets:
        for src in valid_targets:
            if base_variant_to_base[src] == base_variant_to_base[tgt]:
                continue
            for learner in LEARNERS:
                overall_match_ratio = calculate_overall_match_ratio(src, tgt, learner, causal_models,
                                                                    base_variant_to_base)

                for model in causal_models:
                    true_graph = get_causal_model_ground_truth(model, src)
                    pred_graph = get_predicted_graph(tgt, src, learner, [model], base_variant_to_base)

                    same_edges = len(true_graph & pred_graph)

                    match_ratio_detail_records.append({
                        'Source': src,
                        'Target': tgt,
                        'Learner': learner,
                        'Causal_Model': model,
                        'Correct_Predictions': same_edges,
                        'True_Graph_Size': len(true_graph),
                        'Pred_Graph_Size': len(pred_graph),
                        'Jaccard': same_edges / len(true_graph | pred_graph) if len(true_graph) > 0 else 0.0
                    })

                avg_jaccard_cache[(src, tgt, learner)] = overall_match_ratio

    return avg_jaccard_cache, match_ratio_detail_records


def save_jaccard_details(match_ratio_detail_records, output_dir):
    match_ratio_detail_df = pd.DataFrame(match_ratio_detail_records)
    match_ratio_detail_file = os.path.join(output_dir, "match_ratio_detail.csv")
    match_ratio_detail_df.to_csv(match_ratio_detail_file, index=False)
    print(f"✅ Match Ratio saved: {match_ratio_detail_file}")
    return match_ratio_detail_df


def compute_target_jaccard_variance_per_target(match_ratio_detail_df, valid_targets, output_dir):
    target_source_learner_var_map = {}

    for target_full in valid_targets:
        target_related_records = match_ratio_detail_df[
            match_ratio_detail_df['Target'] == target_full
            ]

        if target_related_records.empty:
            continue

        jaccard_var_records = []
        grouped = target_related_records.groupby(['Source', 'Learner'])

        for (src, learner), subdf in grouped:
            mean_jacc = subdf['Jaccard'].mean()
            var_jacc = subdf['Jaccard'].var(ddof=1) if len(subdf) > 1 else 0.0
            jaccard_var_records.append({
                'Source': src,
                'Learner': learner,
                'Mean_Jaccard': mean_jacc,
                'Var_Jaccard': var_jacc,
                'Num_Causal_Models': len(subdf)
            })

        jaccard_var_df = pd.DataFrame(jaccard_var_records)

        jaccard_var_file = os.path.join(output_dir, f"{target_full}_jaccard_variance_per_source_learner.csv")
        jaccard_var_df.to_csv(jaccard_var_file, index=False)

        source_learner_var_map = {}
        for _, row in jaccard_var_df.iterrows():
            source_learner_var_map[(row['Source'], row['Learner'])] = row['Var_Jaccard']

        target_source_learner_var_map[target_full] = source_learner_var_map

    return target_source_learner_var_map


def load_bellwether_cache():
    bellwether_score_cache = {}
    bellwether_cache_dir = "./Bellwether_results"
    bellwether_cache_file = os.path.join(bellwether_cache_dir, "bellwether_cache.csv")
    os.makedirs(bellwether_cache_dir, exist_ok=True)

    if os.path.exists(bellwether_cache_file):
        cache_df = pd.read_csv(bellwether_cache_file)
        for _, row in cache_df.iterrows():
            key = (row['Source'], row['Target'], row['Learner'])
            bellwether_score_cache[key] = float(row['Bellwether_Score'])
    else:
        print(f"\n⚠️ new file: {bellwether_cache_file}")

    return bellwether_score_cache, bellwether_cache_file


def compute_missing_bellwether_scores(projects, valid_targets, base_variant_to_base,
                                      bellwether_score_cache, bellwether_cache_file,
                                      source_causal_features, N_FAST_RUNS):
    all_needed = []
    for tgt in valid_targets:
        for src in valid_targets:
            if base_variant_to_base[src] == base_variant_to_base[tgt]:
                continue
            for learner in LEARNERS:
                all_needed.append((src, tgt, learner))

    missing = [key for key in all_needed if key not in bellwether_score_cache]

    if missing:
        new_records = []
        for i, (src, tgt, learner) in enumerate(missing, 1):
            validation_projects = [p for p in valid_targets if p != src and p != tgt]
            if not validation_projects:
                score = 0.0
            else:
                val_scores = []
                for val_proj in validation_projects:
                    apfd_vals = []
                    for _ in range(N_FAST_RUNS):
                        rs = np.random.randint(0, 100000)
                        model_inst = get_models(rs)[learner]
                        causal_feats = source_causal_features.get(src, None)
                        apfd_c, _, _ = evaluate_transfer_causal(
                            projects[src], projects[val_proj], model_inst, causal_feats
                        )
                        apfd_vals.append(apfd_c)
                    val_scores.append(np.mean(apfd_vals))
                score = np.mean(val_scores) if val_scores else 0.0

            bellwether_score_cache[(src, tgt, learner)] = score
            new_records.append({
                'Source': src,
                'Target': tgt,
                'Learner': learner,
                'Bellwether_Score': score
            })

            if i % 20 == 0 or i == len(missing):
                print(f"  evaluate {i}/{len(missing)}")

        new_df = pd.DataFrame(new_records)
        if os.path.exists(bellwether_cache_file):
            old_df = pd.read_csv(bellwether_cache_file)
            combined_df = pd.concat([old_df, new_df], ignore_index=True)
            combined_df.drop_duplicates(subset=['Source', 'Target', 'Learner'], keep='last', inplace=True)
        else:
            combined_df = new_df

        combined_df.to_csv(bellwether_cache_file, index=False)
        print(f"✅ Bellwether update: {bellwether_cache_file}")
    else:
        print("\n✅ Bellwether Score saved")


def select_best_hybrid_combination(avg_jaccard_cache, bellwether_score_cache,
                                   valid_targets, base_variant_to_base,
                                   target_source_learner_var_map, target_full, VAR_THRESHOLD, ALPHA):
    candidate_records = []

    for source_full in valid_targets:
        if base_variant_to_base[source_full] == base_variant_to_base[target_full]:
            continue
        for learner in LEARNERS:
            jacc = avg_jaccard_cache.get((source_full, target_full, learner), 0.0)
            bellwether_score = bellwether_score_cache.get((source_full, target_full, learner), 0.0)
            var_jacc = target_source_learner_var_map.get(target_full, {}).get((source_full, learner), 0.0)

            candidate_records.append({
                'Source': source_full,
                'Learner': learner,
                'Jaccard': jacc,
                'Bellwether_Score': bellwether_score,
                'Var_Jaccard': var_jacc
            })

    if not candidate_records:
        print(f"  ⚠️ skip {target_full}")
        return None

    total_candidates = len(candidate_records)
    high_variance_candidates = [rec for rec in candidate_records if rec['Var_Jaccard'] > VAR_THRESHOLD]
    high_variance_count = len(high_variance_candidates)

    if high_variance_candidates:
        high_var_df = pd.DataFrame(high_variance_candidates)
        high_var_file = os.path.join(output_dir, f"{target_full}_high_variance_combinations.csv")
        high_var_df.to_csv(high_var_file, index=False)
        print(f"  ✅ {high_var_file}")

    if total_candidates > 0:
        high_variance_ratio = high_variance_count / total_candidates
        stats_record = {
            'Target': target_full,
            'Total_Candidates': total_candidates,
            'High_Variance_Candidates': high_variance_count,
            'High_Variance_Ratio': high_variance_ratio,
            'VAR_THRESHOLD': VAR_THRESHOLD
        }
        stats_file = os.path.join(output_dir, "variance_stats_summary.csv")
        stats_df = pd.DataFrame([stats_record])

        if os.path.exists(stats_file):
            existing_stats = pd.read_csv(stats_file)
            combined_stats = pd.concat([existing_stats, stats_df], ignore_index=True)
            combined_stats.to_csv(stats_file, index=False)
        else:
            stats_df.to_csv(stats_file, index=False)

        print(
            f"  ℹ️ {target_full}: {high_variance_ratio:.4f} ({high_variance_count}/{total_candidates})")

    best_hybrid_score = -1
    best_record = None
    method_used = "Hybrid"

    for idx, rec in enumerate(candidate_records):
        source_full = rec['Source']
        learner = rec['Learner']
        var_jacc = rec['Var_Jaccard']

        if var_jacc > VAR_THRESHOLD:
            bellwether_norm_var = min_max_norm(np.array([rec['Bellwether_Score']]))[0]
            hybrid_score = bellwether_norm_var
        else:
            bellwether_scores = np.array([r['Bellwether_Score'] for r in candidate_records])
            jaccard_scores = np.array([r['Jaccard'] for r in candidate_records])
            bellwether_norm = min_max_norm(bellwether_scores)
            jaccard_norm = min_max_norm(jaccard_scores)
            hybrid_score = ALPHA * bellwether_norm[idx] + (1 - ALPHA) * jaccard_norm[idx]

        if hybrid_score > best_hybrid_score:
            best_hybrid_score = hybrid_score
            best_record = rec
            method_used = "Bellwether_only" if rec['Var_Jaccard'] > VAR_THRESHOLD else "Hybrid"

    best_source_full = best_record['Source']
    best_learner = best_record['Learner']
    best_avg_jacc = best_record['Jaccard']
    best_bellwether_score = best_record['Bellwether_Score']
    target_var = best_record['Var_Jaccard']

    print(f"Source={best_source_full}, Learner={best_learner}, "
          f"Bellwether={best_bellwether_score:.4f}, Jaccard={best_avg_jacc:.4f}, "
          f"Var_Jaccard={target_var:.4f}, Method={method_used}")

    return best_record, method_used, best_hybrid_score

def evaluate_final_performance(projects, best_source_full, best_learner, target_full,
                               source_causal_features, N_FINAL_RUNS):
    apfd_c_list = []
    recall20_list = []
    effort20_list = []

    for run in range(N_FINAL_RUNS):
        rs = np.random.randint(0, 100000)
        model_inst = get_models(rs)[best_learner]
        causal_feats = source_causal_features.get(best_source_full, None)

        apfd_c, recall20, effort20 = evaluate_transfer_causal(
            projects[best_source_full],
            projects[target_full],
            model_inst,
            causal_feats
        )
        apfd_c_list.append(apfd_c)
        recall20_list.append(recall20)
        effort20_list.append(effort20)

    return apfd_c_list, recall20_list, effort20_list


def save_final_results(apfd_c_list, recall20_list, effort20_list, best_source_full,
                       best_learner, best_hybrid_score, best_avg_jacc, best_bellwether_score,
                       target_full, method_used, N_FINAL_RUNS, output_dir):
    detail_df = pd.DataFrame({
        'Run': list(range(1, N_FINAL_RUNS + 1)),
        'APFD_C': apfd_c_list,
        'Recall20': recall20_list,
        'Effort20': effort20_list,
        'Bellwether_Project': [best_source_full] * N_FINAL_RUNS,
        'Learner': [best_learner] * N_FINAL_RUNS,
        'Avg_Jaccard': [best_avg_jacc] * N_FINAL_RUNS,
        'Bellwether_Score': [best_bellwether_score] * N_FINAL_RUNS,
        'Target_Project': [target_full] * N_FINAL_RUNS,
        'Method': [method_used] * N_FINAL_RUNS
    })
    detail_file = os.path.join(output_dir, f"{target_full}_hybrid_runs.csv")
    detail_df.to_csv(detail_file, index=False)


def run_analysis():
    projects, base_variant_to_base, valid_targets = load_projects(data_dir)

    causal_models = get_causal_models()
    print(f"\n✅ causal model: {causal_models}")

    os.makedirs(output_dir, exist_ok=True)

    source_causal_features = compute_causal_features(projects, valid_targets, causal_models)
    save_causal_features(source_causal_features, output_dir)

    avg_jaccard_cache, match_ratio_detail_records = compute_avg_jaccard(
        projects, valid_targets, base_variant_to_base, causal_models
    )
    match_ratio_detail_df = save_jaccard_details(match_ratio_detail_records, output_dir)

    target_source_learner_var_map = compute_target_jaccard_variance_per_target(match_ratio_detail_df, valid_targets,
                                                                               output_dir)

    bellwether_score_cache, bellwether_cache_file = load_bellwether_cache()
    compute_missing_bellwether_scores(projects, valid_targets, base_variant_to_base,
                                      bellwether_score_cache, bellwether_cache_file,
                                      source_causal_features, N_FAST_RUNS)

    summary_records = []

    for target_full in valid_targets:
        print(f"\n🎯 target: {target_full}")

        result = select_best_hybrid_combination(avg_jaccard_cache, bellwether_score_cache,
                                                valid_targets, base_variant_to_base,
                                                target_source_learner_var_map, target_full, VAR_THRESHOLD, ALPHA)

        if result is None:
            continue

        best_record, method_used, best_hybrid_score = result
        best_source_full = best_record['Source']
        best_learner = best_record['Learner']
        best_avg_jacc = best_record['Jaccard']
        best_bellwether_score = best_record['Bellwether_Score']
        target_var = target_source_learner_var_map.get(target_full, {}).get((best_source_full, best_learner), 0.0)

        print(f"  ✅ : Source={best_source_full}, Learner={best_learner}, "
              f"Bellwether={best_bellwether_score:.4f}, Jaccard={best_avg_jacc:.4f}, Method={method_used}")

        apfd_c_list, recall20_list, effort20_list = evaluate_final_performance(
            projects, best_source_full, best_learner, target_full,
            source_causal_features, N_FINAL_RUNS
        )

        save_final_results(apfd_c_list, recall20_list, effort20_list, best_source_full,
                           best_learner, best_hybrid_score, best_avg_jacc, best_bellwether_score,
                           target_full, method_used, N_FINAL_RUNS, output_dir)

        summary_records.append({
            'Source_Project': best_source_full,
            'Learner': best_learner,
            'Target_Project': target_full,
            'Avg_Jaccard': best_avg_jacc,
            'Bellwether_Score': best_bellwether_score,
            'Final_APFD_C_Mean': np.mean(apfd_c_list),
            'Final_Recall20_Mean': np.mean(recall20_list),
            'Final_Effort20_Mean': np.mean(effort20_list),
            'Var_Jaccard_Source_Learner': target_var,
            'Method': method_used
        })

    if summary_records:
        summary_df = pd.DataFrame(summary_records).sort_values(
            ['Target_Project'], ascending=[True]
        )
        summary_file = os.path.join(output_dir, "hybrid_selection_summary.csv")
        summary_df.to_csv(summary_file, index=False)
        print(f"\n✅ Summary file saved to: {summary_file}")
    else:
        print("\n❌ no results")

    print("\n🎉 Hybrid Bellwether + Recall@20 + Effort@20 finished!")


def main():
    global VAR_THRESHOLD, ALPHA, N_FAST_RUNS, N_FINAL_RUNS, data_dir, output_dir
    VAR_THRESHOLD = 0.015
    ALPHA = 0.4
    N_FAST_RUNS = 5
    N_FINAL_RUNS = 30
    data_dir = "../ADS_testing/src/bellwether/data"
    output_dir = f"./CHOSEN_{ALPHA}_{VAR_THRESHOLD}"
    run_analysis()


if __name__ == "__main__":
    main()
